...
std::pair<int, int> get_move(Node* n) {
    for (int i = 0; i < 9; i++) {
        for (int j = 0; j < 9; j++) {
...
Node* get_child(Node* n, int i, int j) {
    uint16_t r = i / 3;
    uint16_t c = j / 3;
    uint16_t sr = i % 3;
    uint16_t sc = j % 3;
...
void expand(Node* n)
{
    ...
    /* std::printf("before: %d children\n", n->num_children); */
    if (s.valid_square == 9 || board_state(s.small_board[s.valid_square]) != unfinished) {
        /* std::printf("BOB\n"); */
        n->children = new Node*[81]();
        /* std::printf("XD\n"); */
            ...
            if (board_state(s.small_board[sq]) == unfinished) {
                ...
                for (int i = 0; i < 9; i++) {
                    if ((moves & (1 << i)) != 0) {
                        ...
                        n->children[n->num_children++] = new Node(new_state, n);
                    }
                }
            }
        }
        /* std::printf("after: %d children\n", n->num_children); */
    } else {
        n->children = new Node*[9]();
        uint16_t sq = s.valid_square;
        uint16_t moves = available_moves(s.small_board[sq]);
        for (int i = 0; i < 9; i++) {
            if ((moves & (1 << i)) != 0) {
                /* std::printf("NEW CHILD 2\n"); */
                State new_state = s;
                new_state.player = next_player;
...
void backpropagate(Node* n, Condition endstate)
{
    Node* nc = n;
    if (endstate == draw) {
        while (nc != nullptr) {
 ...
 */
float UCT(Node* n, float constant)
{
    if (n->visits == 0) {
...
Node* mcts_next_move(Node* root, int time_limit_ms)
{
    ...
    while (c % 16 != 0 || std::chrono::steady_clock::now() - start < std::chrono::milliseconds(time_limit_ms)) {
        ...
        if (leaf->visits != 0) {
            ...
            if (leaf->num_children != 0) {
                ...
                /* leaf = leaf->children[randval(leaf->num_children)]; */
            }
        }
...
int main(int argc, char* argv[])
{
    ...
    std::fprintf(stderr, "node size = %d\n",(int)sizeof(Node));
    State s{};
    Node* root = new Node(s, nullptr);
    ...
    while (1) {
        ...
        if (opp_row != -1) {
            ...
            if (root->num_children == 0) expand(root);
        ...
        Node* next = mcts_next_move(root, first_move ? 1000 : 100);
